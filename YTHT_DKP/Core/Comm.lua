...
----------------------------------------------------------------------
-- 单条日志接收（chunked，批量操作的玩家列表可能很长）
----------------------------------------------------------------------
local pendingLogEntry = {}

local function HandleLogEntryChunk(parts, sender)
    local chunkIndex = tonumber(parts[2])
    local totalChunks = tonumber(parts[3])
    local chunkData = parts[4] or ""
    if not chunkIndex or not totalChunks then return end

    local sKey = sender .. "_logentry"
    if not pendingLogEntry[sKey] then
        pendingLogEntry[sKey] = { chunks = {}, expected = totalChunks }
    end
    local sync = pendingLogEntry[sKey]
    sync.chunks[chunkIndex] = chunkData
    ...
    if received >= sync.expected then
        ...
        if entry.id then
            for _, e in ipairs(DKP.db.log) do
                if e.id == entry.id then return end
            end
        end
...
commFrame:SetScript("OnEvent", function(self, event, ...)
    if event == "CHAT_MSG_ADDON" then
        ...
        elseif msgType == "LOG_ENTRY" then
            -- 单条日志追加（chunked 接收）
            HandleLogEntryChunk(parts, sender)
    ...
    elseif event == "GROUP_ROSTER_UPDATE" then
        if IsInRaid() or IsInGroup() then
            -- 管理员自动切换到管理模式
            if DKP.IsOfficer() and DKP.db and DKP.db.mode ~= "admin" then
                DKP.db.mode = "admin"
                DKP.Print("|cff00FF00检测到管理员身份，已自动切换到管理模式|r")
                if DKP.RefreshDKPUI then DKP.RefreshDKPUI() end
                if DKP.RefreshTableUI then DKP.RefreshTableUI() end
            end
            -- 不再自动广播权限或请求同步（手动点"全量同步"或"同步DKP"）
        end
    end
end)
